...
def create_malleable_app(config: MalleableConfig) -> FastAPI:
    ...
    # Build the endpoint with a concrete annotation so FastAPI can resolve it
    # at route-registration time (avoids the PEP 563 lazy-annotation trap).
    def _make_chat_endpoint(req_model, ui_schema_model):
        async def chat(body: req_model):  # type: ignore[valid-type]
            ...
            # Generation path
            schema_obj = _ensure_obj(current_schema, ui_schema_model) if ui_schema_model and ui_schema_model is not dict else current_schema_dict
            ...
            if parsed.get("action") == "schema" and parsed.get("schema"):
                try:
                    if ui_schema_model and ui_schema_model is not dict:
                        result["schema"] = ui_schema_model(**parsed["schema"]).model_dump()
                    else:
        ...
        # Give the inner function a unique name so FastAPI's operation ID doesn't collide
        chat.__name__ = "chat"
        chat.__annotations__ = {"body": req_model, "return": dict}
        return chat

    app.post("/chat")(_make_chat_endpoint(ChatReq, UISchemaModel))
