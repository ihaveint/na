    ...
    <>
      ...
      <div className="fixed inset-0 md:inset-auto md:right-0 md:top-0 md:h-full md:w-[300px] bg-white border-l border-zinc-200 shadow-2xl flex flex-col z-[9999]">
        <div className="px-4 py-3 border-b border-zinc-100 flex items-center justify-between flex-shrink-0">
          ...
          <button
            onClick={onClose}
            className="text-zinc-400 hover:text-zinc-700 w-6 h-6 flex items-center justify-center rounded hover:bg-zinc-100 text-lg leading-none"
          >
            ×
          </button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto min-h-0">
          {displayed.length === 0 ? (
            <p className="text-xs text-zinc-400 text-center py-12 px-4">
                  ...
                  <div key={v.id} className="flex gap-3 px-4 py-3 group">
                    ...
                    <div className="flex-1 min-w-0 pb-1">
                      <div className="flex items-start justify-between gap-2">
                        ...
                        {isCurrent && (
                          <span className="text-[10px] font-semibold text-violet-600 bg-violet-50 px-1.5 py-0.5 rounded flex-shrink-0">
                            current
                          </span>
                        )}
                        {isFuture && (
                      ...
                      <div className="flex items-center justify-between mt-1">
                        <div className="flex items-center gap-1.5">
                          <span className={`text-[10px] px-1.5 py-0.5 rounded font-medium ${
                            ...
                            {v.renderMode === "component" ? "custom" : v.schema.layout}
                          </span>
