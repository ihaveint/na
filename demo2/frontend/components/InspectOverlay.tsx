...
export default function InspectOverlay({ active, onElementClick }: Props) {
  ...
  function handleMouseMove(e: React.MouseEvent) {
    ...
    if (!el || el === document.body || el === document.documentElement) {
      setHighlight(null); setTooltip(null); return
    }
  ...
  function handleMouseLeave() { setHighlight(null); setTooltip(null) }

  function handleClick(e: React.MouseEvent) {
    e.preventDefault(); e.stopPropagation()
    const el = getElementUnder(e.clientX, e.clientY)
    if (!el) return
    setHighlight(null); setTooltip(null)
    ...
    <>
      ...
      {highlight && (
        <div
          ...
          style={{
            zIndex: 99999,
            top: highlight.top - 2, left: highlight.left - 2,
